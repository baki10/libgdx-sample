...
public class ShapeRendererScreen extends AbstractScreen {
  ...
  public void show() {
    ...
    debugFunction = initFunction();

    camera.position.set(0.0f, 0.0f, 0.0f);

    Gdx.app.log("ShapeRendererScreen", "G: toggle grid");
    Gdx.app.log("ShapeRendererScreen", "F: toggle function y = x^2");
    Gdx.app.log("ShapeRendererScreen", "C: toggle circles");
    Gdx.app.log("ShapeRendererScreen", "R: toggle rectangles");
    Gdx.app.log("ShapeRendererScreen", "P: toggle points");
    Gdx.app.log("ShapeRendererScreen", "T: toggle triangles");

    Gdx.input.setInputProcessor(this);
  }

  private float[] initFunction() {
    float[] func = new float[202];

    int i = 0;
    for (float x = -10; x < 10; x = x + 0.2f) {
      func[i] = x;
      func[i + 1] = x * x;
      i += 2;
    }
  ...
  private void drawDebugGraphics() {
    ...
    if (drawRectangles) {
      ...
      shapeRenderer.rect(7.2f, 2.4f, 3.3f, 2.8f);
      shapeRenderer.rect(-8.4f, 3.8f, 6.1f, 2.3f);
      shapeRenderer.rect(-4.2f, -3.4f, 3.3f, 2.8f);
      shapeRenderer.rect(3.2f, -6.4f, 3.9f, 1.8f);
      shapeRenderer.end();
    }
